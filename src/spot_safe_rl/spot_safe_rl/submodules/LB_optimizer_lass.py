...
class FhFunction:
    ...
    def h(self, x): 
        ...
        if cl_obs_1 is not None and cl_obs_2 is not None and cl_obs_3 is not None:
            ...
            get_away = 0.01 * np.array([sum(length_to_obs_1)/len(length_to_obs_1), sum(length_to_obs_2)/len(length_to_obs_2), sum(length_to_obs_3)/len(length_to_obs_3)])

        # if closest_obstacle is not None:
        #     for point in closest_obstacle:
        #         length_to_obs.append( 2 - np.linalg.norm(np.array(point) - np.array(x)[:2]))
...
class Simulation:
    ...
    x0: np.array = None
    M0: float = 0.5 / d
    Ms: np.array = 0.2 * np.ones(m) # Ms: np.array = 0.152 * np.ones(m)
